...
FROM (
    SELECT year, flow_type, ROUND(SUM(ratio), 2) AS sum_ratio
    FROM nlic_chung_regional_cargo_flow
...
SELECT 
    '2. NULL 또는 0 이하 물동량 행 수' AS check_item,
...
SELECT 
    ...
    COUNT(DISTINCT base_region_kr) AS unique_base_cities, -- 출발 시군구 수 (중복제거)
    COUNT(DISTINCT target_region_kr) AS unique_target_cities, -- 도착 시군구 수 (중복제거)
