...
class ColorMapper:
    ...
    def get_color(self, region):
        if region not in self.color_map:
            self.color_map[region] = self.palette[
...
def prepare_stacked_data(df, f_type, threshold=10.0):
    ...
    for yr in years:
        ...
        grouped_df.rename(
            columns={"processed_region": "region"}, inplace=True
        )
...
def generate_multilingual_plots(df_long, data_df, save_dir):
    for lang in ["ko", "en"]:
        ...
        for i, gubun in enumerate(["출발", "도착"]):
            ...
            df_stacked = prepare_stacked_data(
                df_long, f_type=gubun, threshold=10.0
            )
        ...
        fig.suptitle(
            cfg["title_main"], fontsize=17, fontweight="bold", y=0.98
        )
