...
# 데이터 로드
df = pd.read_csv(data_dir / 'national_freight_weight.csv')
...
I18N = {
    'ko': {
        'filename': 'national_intra_vs_inter_summary_ko.png',
    ...
    'en': {
        'filename': 'national_intra_vs_inter_summary_en.png',
...
def generate_summary_plots():
    for lang, cfg in I18N.items():
        ...
        ax1.set_ylabel(
            cfg['ylabel_left'], fontsize=12, fontweight='bold', labelpad=10
        )
        ax1.set_xlabel(
            cfg['xlabel'], fontsize=12, fontweight='bold', labelpad=10
        )
        ax1.set_xticks(x)
        ...
        ax1.legend(
            lines1 + lines2, labels1 + labels2, loc='upper right', frameon=True
        )

        plt.title(
            cfg['title'], fontsize=14, fontweight='bold', pad=15
        )
        plt.tight_layout()
