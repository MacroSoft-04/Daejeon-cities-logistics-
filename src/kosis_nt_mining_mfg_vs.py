...
* Chart: Daejeon manufacturing indexed to the cleaning step's base year,
  showing establishments falling while revenue rises.
...
df = pd.read_csv(SRC)
subset = df[df["산업별"] == INDUSTRY]

indexed = subset.pivot(index="연도", columns="지표", values="지수").sort_index()
raw = subset.pivot(index="연도", columns="지표", values="값").sort_index()
...
# Leave room on the right so the end-of-line labels are not clipped.
...
note = (
    f"{first}→{last} 사업체수 {raw.loc[last, '사업체수'] - raw.loc[first, '사업체수']:+,.0f}개, "
