...
# 기존 코드 수정: partner_city_en을 region으로 변경, cargo_volume을 vol로 변경
if "partner_city_en" in df_long.columns:
    df_long = df_long.rename(columns={"partner_city_en": "region"})
elif "target_region" in df_long.columns:
    df_long = df_long.rename(columns={"partner_region": "region"})

if "cargo_volume" in df_long.columns:
    df_long = df_long.rename(columns={"cargo_volume": "vol"})

# 수치형 변환 (숫자로 변환되지 않는 값은 NaN 처리 후 0으로 채움)
df_long["vol"] = pd.to_numeric(df_long["vol"], errors="coerce").fillna(0)
df_long["ratio"] = pd.to_numeric(df_long["ratio"], errors="coerce").fillna(0)
...
def get_region_info(key, lang="en"):
    # region 컬럼에서 key 검색
    match = df_long.loc[df_long["region"] == key]
    if not match.empty:
        # 한국어 출력 시 partner_city_kr 사용, 영어 출력 시 key(region) 그대로 사용
        return match.iloc[0]["partner_city_kr"] if lang == "ko" else key
    return key
...
def prepare_stacked_data(df, f_type, threshold=10.0):
    ...
    filtered_df = df[df["direction"] == f_type].copy()
    ...
    for yr in years:
        ...
        grouped_df = yr_df.groupby(
            ["year", "direction", "processed_region"], as_index=False
        )[["ratio", "vol"]].sum()
